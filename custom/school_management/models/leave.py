...
class Leave(models.Model):
    ...
    number_of_days = fields.Float('Duration (Days)', compute='_compute_number_of_days', store=True)
    is_half_day = fields.Boolean('Is Half Day')
    half_day = fields.Selection([('fn','Forenoon'),('an','Afternoon')])
    reason = fields.Html()
    ...
    def _compute_number_of_days(self):
        ...
        if isinstance(self.date_from, date) and isinstance(self.date_to, date):
            #total_days= (self.date_to - self.date_from).days
            work_days = 0
            ...
            while current_date <= self.date_to:
                if current_date.weekday() not in [6, 7]:
                    work_days += 1
                current_date += timedelta(days=1)

            self.number_of_days = work_days if not self.is_half_day else 0.5
